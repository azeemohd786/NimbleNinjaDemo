...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func addHero() {
      ...
      //  hero.position = CGPointFloat16(hero.frame.size.height);Int(movingGround.position.y);Float(movingGround.position.y + movingGround.frame.size.height/2 + Int(Float16(hero.frame.size.height)))
        addChild(hero)
        hero.breathe()
    }
 //===========================
